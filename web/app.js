...
(() => {
  ...
  function renderApiKeys() {
    ...
    const html = apiKeysCache.map(item => {
      ...
      const boundCount = Array.isArray(item.boundAccounts) ? item.boundAccounts.length : 0;
      const bound = boundCount > 0
        ? '<span class="text-xs" style="background:rgba(16,185,129,0.15);color:#10b981;padding:1px 6px;border-radius:4px;">' + escapeHtml(t('apiKeys.boundBadge', boundCount)) + '</span>'
        : '';
  ...
  function openApiKeyModal(entry) {
    ...
    renderApiKeyBoundAccounts(entry && Array.isArray(entry.boundAccounts) ? entry.boundAccounts : []);
    apiKeyModalSubmitting = false;
    $('apiKeyModalSaveBtn').disabled = false;
    openDialog('apiKeyModal');
  }

  // Render the account-binding checkbox list inside the API key modal.
  // selectedIds are the account IDs currently bound to the key being edited.
  function renderApiKeyBoundAccounts(selectedIds) {
    const box = $('apiKeyForm_boundAccounts');
    if (!box) return;
    const selected = new Set(selectedIds || []);
    if (!accountsData.length) {
      box.innerHTML = '<div class="muted-text" style="padding:0.25rem 0;">' + escapeHtml(t('apiKeys.boundEmpty')) + '</div>';
      return;
    }
    box.innerHTML = accountsData.map(a => {
      const id = escapeAttr(a.id);
      const label = a.email ? escapeHtml(a.email) : escapeHtml(a.id);
      const checked = selected.has(a.id) ? ' checked' : '';
      return '<label class="flex items-center gap-2" style="padding:2px 0;cursor:pointer;">' +
        '<input type="checkbox" data-apikey-bound="1" value="' + id + '"' + checked + ' /> ' +
        '<span class="text-xs">' + label + '</span>' +
        '</label>';
    }).join('');
  }

  // Collect the account IDs checked in the binding list.
  function collectApiKeyBoundAccounts() {
    const box = $('apiKeyForm_boundAccounts');
    if (!box) return [];
    return Array.from(box.querySelectorAll('input[data-apikey-bound="1"]:checked')).map(cb => cb.value);
  }
  ...
  async function submitApiKeyModal() {
    ...
    try {
      ...
      const payload = {
        ...
        boundAccounts: collectApiKeyBoundAccounts()
      };
      let res, d;
      if (apiKeyEditingId) {
